...
class Main {
    ...
    public static void main(String[] args) {
        System.out.println("Henlo, worlnd");
    ...
    static void turn() throws Exception {
        ...
        int chip = decodeChipID(input.nextLine());
        flip(chip);
    }
    ...
    static int decodeChipID(String chip) throws Exception {
        ...
        if (
            ...
            (int) chip.charAt(1) - '1' > 2          // Digit is not valid
    ...
    static void printMatrix() {
        char[] headerRow = {'A', 'B', 'C'};

        System.out.println(matrixHead);
        printRow(' ', headerRow);

        for (int row = 0; row < 3; row++) {
            System.out.println(matrixDivider);
            
            char[] rowSym = new char[3];
            
            for (int chip = 0; chip < 3; chip++) {
                rowSym[chip] = matrix[row][chip] ? 'X' : 'O';
            }

            printRow((char) (row + 1 + '0'), rowSym);
        }

        System.out.println(matrixFoot);
    }
